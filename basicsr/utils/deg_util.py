...
class SRMDPreprocessing(object):
    def __init__(
        ...
        stored_kernel=False, pre_kernel_path = None , use_ker_map=False
    ):
        if use_ker_map:
    ...
    def __call__(self, hr_tensor, kernel=False):
        ...
        hr_var = Variable(hr_tensor)
        lr=b_Bicubic(hr_var, self.scale)
        ...
        return (lr_re, re_code, b_kernels,hr_blured_var,lr) if kernel else (lr_re, re_code)
