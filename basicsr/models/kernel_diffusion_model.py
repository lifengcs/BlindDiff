...
from basicsr.kernel_diffusion import create_diffusion
from basicsr.kernel_diffusion.timestep_sampler import create_named_schedule_sampler
...
class KDiffusionModel(BaseModel):
    """Blind SR model for blind single image super-resolution."""

    def __init__(self, opt):
        super(KDiffusionModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        t, weights = self.schedule_sampler.sample(self.gt.shape[0], self.device)
        loss = self.diffusion.training_losses(self.net_g,self.gt,t,gt_ker=self.ker)
        l_total = (loss["loss"] * weights).mean()
        print(weights)
        input()
        #print(l_total)
        loss_dict = OrderedDict()
        loss_dict['mse'] = loss["mse"]
        loss_dict['vb'] = loss["vb"]
        # pixel loss
        l_total.backward()
    ...
    def test(self):
        ...
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                z = torch.randn([b,c,h,w]).to(self.device)
                self.output = self.diffusion.p_sample_loop(self.net_g_ema,z.shape,z,clip_denoised=False, progress=True, device=self.device)
        else:
            self.net_g.eval()
            with torch.no_grad():
                z = torch.randn([b,c,h,w]).to(self.device)
                self.output = self.diffusion.p_sample_loop(self.net_g,z.shape,z,clip_denoised=False, progress=True, device=self.device)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            sr_img = 1 / (np.max(sr_img)) * 255 * sr_img
