...
class DiffusionModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
       ...
       # else:
        loss = self.diffusion.training_losses(self.net_g,self.gt,t,model_kwargs)
        l_total = (loss["loss"] * weights).mean()
        #print(l_total)
        loss_dict = OrderedDict()
        loss_dict['mse'] = loss["mse"]
        #loss_dict['ker_2'] = loss["ker_2"]
        loss_dict['vb'] = loss["vb"]
        #loss_dict['kl'] = loss["kl"]
        # pixel loss
        l_total.backward()
    ...
    def test(self):
        ...
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            self.output = self.diffusion.p_sample_loop(self.net_g_ema,z.shape,z,clip_denoised=False, model_kwargs=model_kwargs, progress=True, device=self.device)# + F.interpolate(model_kwargs["cond"], scale_factor=4, mode='bicubic')
                #print(samples.shape)
                #input()
