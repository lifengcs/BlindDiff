...
class DiffusionModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        loss = self.diffusion.training_losses(self.net_g,self.gt,t,model_kwargs,gt_ker=self.ker,opt = self.opt)
    ...
    def test(self):
        ...
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            self.output = self.diffusion.p_sample_loop(self.net_g_ema,z.shape,z,clip_denoised=False, model_kwargs=model_kwargs, progress=True, device=self.device,grad_opt=self.grad_opt)# + F.interpolate(model_kwargs["cond"], scale_factor=4, mode='bicubic')
                #print(samples.shape)
                #input()
            ...
            self.net_g.eval()
            self.output = self.diffusion.p_sample_loop(self.net_g,z.shape,z,clip_denoised=False, model_kwargs=model_kwargs, progress=True, device=self.device,grad_opt=self.grad_opt)
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if with_metrics:
                ...
                for name, opt_ in self.opt['val']['metrics'].items():
                    if name == 'lpips':
                        opt_['loss_fn_vgg'] = self.loss_fn_vgg
