...
from basicsr.diffusion2 import create_diffusion
from basicsr.diffusion2.timestep_sampler import create_named_schedule_sampler
from .base_model import BaseModel
from torch.nn import functional as F
import time

@MODEL_REGISTRY.register()
class DiffusionModel2(BaseModel):
    """Blind SR model for blind single image super-resolution."""

    def __init__(self, opt):
        super(DiffusionModel2, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        loss = self.diffusion.training_losses(self.net_g,self.gt,t,model_kwargs,gt_ker=self.ker)
        l_total = (loss["loss"] * weights).mean()
        #print(l_total)
        loss_dict = OrderedDict()
        loss_dict['mse'] = loss["mse"]
        #loss_dict['vb'] = loss["vb"]
        loss_dict['ker'] = loss["ker"]
        loss_dict['sr'] = loss["sr"]
        loss_dict['mean'] = loss["mean"]
        # pixel loss
        l_total.backward()
    ...
    def test(self):
        # for swinDF
        window_size = 8#self.opt['network_g']['window_size']
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                ...
                #model_kwargs = dict(cond=self.lq)
                self.output = self.diffusion.p_sample_loop(self.net_g_ema,z.shape,z,clip_denoised=False, model_kwargs=model_kwargs, progress=True, device=self.device)
                #print(samples.shape)
                #input()
            ...
            self.net_g.eval()
            with torch.no_grad():
                z = torch.randn(self.gt.shape).to(self.device)
                #model_kwargs = dict(lq=self.lq)
